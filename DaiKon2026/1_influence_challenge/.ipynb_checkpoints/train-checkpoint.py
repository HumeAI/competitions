"""Training entry point for the Influence sub-challenge baseline."""
...
def main():
    ...
    datamodule = InfluenceDataModule(config)
    datamodule.setup()

    model = InfluenceBaseline(config, input_dim=datamodule.input_dim)
    total_params = sum(p.numel() for p in model.parameters())
    print(f"Parameters:    {total_params:,}")

    logger = False if args.no_wandb else WandbLogger(
        project="acii-daikon",
        name=f"influence_{modality}_{run_id}",
        config=config,
    )

    callbacks = [
        EarlyStopping(
            monitor="val_mean_ccc",
            patience=config["training"]["early_stopping_patience"],
            mode="max",
        ),
        ModelCheckpoint(
            dirpath=ckpt_dir,
            filename="best-{epoch}-{val_mean_ccc:.4f}",
            monitor="val_mean_ccc",
            mode="max",
            save_top_k=1,
        ),
